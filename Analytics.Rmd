...
- Gender: Male or female. (dummy variable)
...
plot_usmap(data = mydata2, values = "sarate", lines = "red") + 
  ...
  ) + theme(legend.position = "right") + labs(title="Nationwide School Attendance Rate 2000 by State", caption = sourceText)+ theme(plot.title = element_text(hjust=0.5),plot.caption = element_text(hjust=0))
...
titleText2='The Estimated Race Distribution \n of Kids Aged 3-4 in United States'
...
base= ggplot(data = tableFreq, 
             aes(x = race,
                 y = ratio))
plot1 = base + geom_bar(stat = 'identity')+ theme_minimal()
...
titleText3='The Estimated Gender Distribution \n of Kids Aged 3-4 in United States'
...
titleText4="The Estimated Household Income Level \n of Kids Aged 3-4 in United States"
...
Lastly, parents’ highest education level is shown in the following bar chart. It shows that the highest education level of 33.57% of households is grade 12. Among 53.29% of households, there is at least one parent has received college education.

<br>

```{r, echo=FALSE, fig.align='center'}
absoluteT2=table(mydata$educhh,exclude = 'nothing')
ToPlot2=prop.table(absoluteT2)*100


titleText5='The Estimated Highest Education \n of Parents of Kids Aged 3-4 in United Sates'

tableFreq=as.data.frame(ToPlot2)
names(tableFreq)=c("grade","ratio")
library(ggplot2)
base= ggplot(data = tableFreq, 
             aes(x = grade,
                 y = ratio))
plot1 = base + geom_bar(stat = 'identity')+ theme_minimal()
plot2 = plot1 + labs(title=titleText5,
                     x =NULL, 
                     y = NULL,
                     caption = sourceText)
plot3 = plot2 + scale_y_continuous(limits = c(0, 40),
                                 labels=scales::unit_format(suffix = '%')) 
plot4 = plot3 + theme(plot.caption = element_text(hjust = 0),
                    plot.title = element_text(hjust = 0.5),
                    axis.text.x = element_text(size=9,angle = 0),
...
titleText6='School Attendance Rate for Different Races'
...
bars2 + labs(title=titleText6,
                    ...
                    axis.text.x = element_text(size=9,angle = 0),
                    axis.ticks.length=unit(0,"cm"),
                    axis.ticks=element_blank())+xlab("Race")
...
titleText7='School Attendance Rate for Different Genders'
...
titleText8='School Attendance Rate for Different Income Levels'
...
bars2 + labs(title=titleText8,
                    ...
                    plot.title = element_text(hjust = 0.5),
                    axis.text.x = element_text(size=9,angle = 0),
                    axis.ticks.length=unit(0,"cm"))+geom_vline(aes(xintercept=2.5),linetype=5,col="red") +geom_text(aes(x=2.5, label="Poverty Threshold", y=0.55), colour="blue", angle=90,vjust=1.2,cex=3)
```
...
We used the logit model to analyze effects of race, gender, income, and parents’ education on children’s school attendance. The result shows that parents’ highest education and household income level have a significant correlation with the dependent variable.
...
dwplot(mylogit_t)+labs(title="Regression Result", caption=sourceText)+ geom_vline(xintercept = 0, colour = "grey60",linetype = 2) +scale_colour_grey(start = .1, end = .7)+theme(plot.title = element_text(hjust = 0.5),plot.caption = element_text(hjust=0))
```
